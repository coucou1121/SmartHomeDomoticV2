...
//        if(this->_myPlotStyle != GlobalEnumerate::PLOT_STYLE_WEATHER_STATION)
    {
        for (int i = 1; i <= this->_nbOfTrace; i++)
        {
...
//        GlobalStyle::setStylePlot(this->_customPlot, true);
    ...
    case GlobalEnumerate::PLOT_STYLE_CONSO_TRACE:
        ...
        GlobalStyle::setStyleStatisticPlot(this->_customPlot, false);
        this->_graph1->setLineStyle(QCPGraph::lsImpulse);
        break;
    default:
        break;
    }
...
void CustomPlotItem::_reloadValue()
{
    ...
    if (filePlotsData.open(QFile::ReadOnly))
    {
        ...
        while (!stream.atEnd())
        {
            ...
            else if(this->_myPlotStyle == GlobalEnumerate::PLOT_STYLE_ANANLOG_TRACE)
            {
