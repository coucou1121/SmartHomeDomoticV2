...
public:
    ...
    Q_INVOKABLE void setupStyle(GlobalEnumerate::E_PlotStyle plotstyle, quint8 nbOfTrace,
                                GlobalEnumerate::E_PlotStyle traceColor = GlobalEnumerate::PLOT_STYLE_NONE);

    //set plot label for tank
    Q_INVOKABLE void setupGraphTankLabel(quint8 traceNumber,QString titleText);

    //set plot label if plot as one trace
    Q_INVOKABLE void setupGraphLabel(QString titleText);
    ...
    Q_INVOKABLE void addYValue(qreal valueGraph0, qreal valueGraph1, qreal valueGraph2,
                               ...
                               bool tickAsDate = true, quint8 tickNumber = 0, bool isTheFirstData = false);
...
private:
    ...
    QCPAxisRect *_wideAxisRectRoomData;
    QCPAxisRect *_wideAxisRectHumidity;
    QCPAxisRect *_wideAxisRectPressure;
    ...
    quint8 _yValue;
    quint16 _yValueMin;
    quint16 _yValueMinTemperature;
    quint16 _yValueMax;
    quint16 _yValueMaxTemperature;
    ...
    void setupTraceAnalogTrace(quint8 traceNumber = 1);
    void setupTraceAnalogTrace(GlobalEnumerate::E_PlotStyle plotstyle);
    void setupTraceStatisticTrace(GlobalEnumerate::E_PlotStyle plotstyle);

    void setupTraceStyle(GlobalEnumerate::E_PlotStyle plotstyle, QCPGraph *TraceTop, QCPGraph *TraceAbove = nullptr);
    //set plot style

    //automatically set three traces (temperature in red, humidity in blue with shadow, pressure in yellow)
