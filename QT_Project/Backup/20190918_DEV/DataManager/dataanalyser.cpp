...
DataAnalyser::DataAnalyser(quint16 nbDataFramToTempSave,
    ...
    _filePlotsData(GlobaleStaticValue::savePlotDataFile),
...
void DataAnalyser::savePlotData(DataFrame *dataFrame)
{
    ...
    QTextStream stream(&this->_filePlotsData);


    if (dataFrame->dataFrameState() == 31 && this->_filePlotsData.open(QFile::WriteOnly|QFile::Append))
    {
        stream << this->_dateTime.toMSecsSinceEpoch() << ", " <<  dataFrame->BME280_temperature() << ", " << dataFrame->BME280_humidity() << ", "
               << dataFrame->BME280_pressure() << ", " << dataFrame->ADS115_1_chan0() <<  ", "
               << dataFrame->ADS115_1_chan1() << ", " << dataFrame->ADS115_1_chan2() << ", " << dataFrame->ADS115_1_chan3() << endl;

        this->_filePlotsData.close();
    }
}
...
void DataAnalyser::_deleteHourDirectory(QDateTime *dateTimeToDelete)
{
    ...
    //    if(!directoryExiste)
    //    {
    //        directoryExiste = QDir().remove(this->_directoryDataMain);
    //    }
    //    //check year directory existe
    //    directoryExiste = QDir(_directoryFileNameYear).exists();
