...
void DataAnalyser::run()
{
    ...
    while(true)
    {
        ...
        if(this->_dataFrame->ADS115_1_chan0() > 0)
        {
            ...
            offsetLiquidePressure = this->_offsetVolumePressure((this->_ATMAvg - this->_dataFrame->BME280_pressure()),
...
void DataAnalyser::_updateDataForTheHour(quint8 minuteToUpdate)
{
    ...
    if(volumeConsoTotal > 0)
    {
        volumeConsoTotal = volumeConsoTotal > 0.1 ? 0 : volumeConsoTotal;
    }
    else
    {
        volumeConsoTotal = volumeConsoTotal < -0.1 ? volumeConsoTotal : 0;
    }
