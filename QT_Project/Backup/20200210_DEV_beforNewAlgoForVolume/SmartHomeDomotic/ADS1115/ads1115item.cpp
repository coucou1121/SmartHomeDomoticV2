...
//                         QObject *ADS1115Display,
    ...
    _smoothPlot(NB_OF_VALUE_FOR_SMOOTH_PLOT),
//    _PIDVolumeCoef(1),
...
//                 << this->_timeInMilisecond - this->_memotimeInMilisecond << "[ms]";
        this->_isReadyToStop = !this->askToRun();
...
bool ADS1115Item::isReadyToStop() const
{
    return _isReadyToStop;
}

void ADS1115Item::setIsReadyToStop(bool isReadyToStop)
{
    _isReadyToStop = isReadyToStop;
}

bool ADS1115Item::askToRun() const
{
    return _askToRun;
}

void ADS1115Item::setAskToRun(bool askToRun)
{
    _askToRun = askToRun;
}

qint8 ADS1115Item::smoothPlot() const
{
    return _smoothPlot;
}

void ADS1115Item::setSmoothPlot(const qint8 &smoothPlot)
{
    _smoothPlot = smoothPlot;
}
   ...
   qint16 dataframeState = this->_dataFrame->dataFrameState();
    ...
    else
    {
        if(this->chan0_used())
        {
            ...
            chan0 = this->_meanList(_filterListeChan0, chan0);
            this->_filterList0_IsFull = _filterListeChan0.size() >= this->_smoothPlot ? true : false;
        ...
        if(this->chan1_used())
        {
            ...
            chan1 = this->_meanList(_filterListeChan1, chan1);
            this->_filterList1_IsFull = _filterListeChan1.size() >= this->_smoothPlot ? true : false;
...
double ADS1115Item::_meanList(QList<double> &listToMean, double value)
{
    ...
    while(listToMean.size() > this->_smoothPlot)
    {
...
//    qDebug() << Q_FUNC_INFO << "mean as double : " << (float)meanvalue/listToMean.size() << "listToMean.size(): " << listToMean.size()
//             << "_smoothPlot: " << this->_smoothPlot;
