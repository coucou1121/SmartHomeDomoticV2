...
Item
{
    ...
    property bool memoWarningActive: false
    property bool isVisibleAtStartup : false
    property bool isSelectedOnSetting : false

    property int delayBeforSendFirstMessage: 0

    signal sendWarningLowLevel(string objectName, string objectTitle, int errorNumber, bool warningActive)
    signal sendWarningLowLevelToSetting(string  objectName, bool warningActive)
    ...
    {
        //        console.log(delayBeforSendFirstMessage)
        if(delayBeforSendFirstMessage)
        {
            delayBeforSendFirstMessage = delayBeforSendFirstMessage - 1
        }
        else
        {
            if(isSelectedOnSetting)
            {
            ...
            if(memoWarningActive != warningActive)
            {
