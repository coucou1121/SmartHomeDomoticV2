...
void DataAnalyser::saveTempData(DataFrame *dataFrame)
{
    ...
    checkDataFrameNotEmpty =        dataFrame->BME280_temperature() + dataFrame->BME280_humidity() + dataFrame->BME280_pressure() + dataFrame->ADS115_1_chan0() +
            dataFrame->ADS115_1_chan1() +dataFrame->ADS115_1_chan2() + dataFrame->ADS115_1_chan3();
...
void DataAnalyser::savePlotData(DataFrame *dataFrame)
{
    ...
    qDebug() << Q_FUNC_INFO << listSize << ", :liste size:" << this->_listeOfData.size();

    if(this->_listeOfData.size() > 8460)
    {
        while(this->_listeOfData.size() > 8460)
        {
    ...
    this->_filePlotsDataReadOnly.setPermissions(QFileDevice::ReadOwner |
                                                //                                                QFileDevice::ReadGroup |
                                                QFileDevice::WriteOwner |
                                                //                                                QFileDevice::WriteGroup |
                                                QFileDevice::ExeOwner);
    this->_filePlotsDataWriteOnly.setPermissions(QFileDevice::ReadOwner |
                                                 //                                                 QFileDevice::ReadGroup |
                                                 QFileDevice::WriteOwner |
                                                 //                                                 QFileDevice::WriteGroup |
                                                 QFileDevice::ExeOwner);

    qDebug() << Q_FUNC_INFO << "_filePlotsDataReadOnly permission :" << _filePlotsDataReadOnly.permissions();
    qDebug() << Q_FUNC_INFO << "_filePlotsDataWriteOnly permission :" << _filePlotsDataWriteOnly.permissions();
...
void DataAnalyser::_updateDataForTheDay(QDateTime *dateToUpdate)
{
    ...
    fileDay.setPermissions(QFileDevice::ReadOwner |
                           QFileDevice::ReadGroup |
