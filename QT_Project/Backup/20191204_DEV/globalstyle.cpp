...
//   customPlot->yAxis->setTickLabelColor(_axisColorPlot);
   ...
   if(customPlot->axisRect(0)->axis(QCPAxis::atLeft, 1)!=nullptr)
   {
        customPlot->axisRect(0)->axis(QCPAxis::atLeft, 1)->setTickLabelFont(QFont(QFont().family(), 10));
   }
   else
   {
         customPlot->yAxis->setTickLabelColor(_axisColorPlot);
   ...
   if(customPlot->axisRect(0)->axis(QCPAxis::atLeft, 2)!=nullptr)
   {
        customPlot->axisRect(0)->axis(QCPAxis::atLeft, 2)->setTickLabelFont(QFont(QFont().family(), 10));
   }
...
void GlobalStyle::setTraceColorConsoPlot(QCPGraph *graph)
{
    ...
    graph->setPen(QPen(QBrush(_traceColorConsoPlot),50));
}
...
QColor GlobalStyle::_traceColorHumidityMinPlot(HUMIDITY_UNDER_PLOT_TRACE);
...
//tanks trace
 ...
 //push button
 QColor GlobalStyle::_pushbutton_ON(BACKGROUD_COLOR_PUSHBUTTON_ON);
 QColor GlobalStyle::_pushbutton_OFF(BACKGROUD_COLOR_PUSHBUTTON_OFF);
